...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussxuexitiandiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussxuexitiandiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussxuexitiandi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussxuexitiandi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussxuexitiandi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussxuexitiandi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussxuexitiandi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussxuexitiandi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussxuexitiandi', this.detail);
				this.$utils.msg('审核成功');
