	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussxuexitiandi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussxuexitiandi`, this.ruleForm);
				}else{
					await this.$api.add(`discussxuexitiandi`, this.ruleForm);
				}
				uni.setStorageSync('discussxuexitiandiCleanType',true);
